"""Адаптер локального API Ollama к общему контракту LLMProvider."""
...
class OllamaProvider:
    """Текущая локальная реализация для Qwen через Ollama /api/chat."""
    ...
    def clarification_question(self, unresolved: list[dict[str, Any]], fallback: str) -> str:
        """Ask the model to phrase a question, never to decide a price or a rule."""
        prompt = (
                ...
                "properties": {
                    "items": {
                        ...
                        "items": {
                            ...
                            "required": [
                                "source_ref", "name", "quantity", "width_m", "height_m", "area_m2",
                                "system", "fabric", "color", "opacity", "split_into",
                            ],
                        },
                    }
                },
